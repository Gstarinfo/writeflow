import { WordEntry, POS, VocabularyLevel, SynonymCategory } from './types.js';

// Base dictionary with detailed multi-sense synsets
const CORE_DB: Record<string, WordEntry[]> = {
  "important": [
    {
      lemma: "important",
      pos: "a",
...
export function lemmatizeWord(word: string): { lemma: string; posCandidate?: POS } {
  ...
  if (lower.endsWith('ies') && lower.length > 4) {
    ...
    if (CORE_DB[candidate]) return { lemma: candidate };
  }
  ...
  if (lower.endsWith('ed') && lower.length > 3) {
    ...
    if (CORE_DB[candidate1]) return { lemma: candidate1, posCandidate: 'v' };
    const candidate2 = lower.slice(0, -1);
    if (CORE_DB[candidate2]) return { lemma: candidate2, posCandidate: 'v' };
  }
  ...
  if (lower.endsWith('ly') && lower.length > 3) {
    const candidate = lower.slice(0, -2);
    if (CORE_DB[candidate]) return { lemma: candidate, posCandidate: 'a' };
  }
