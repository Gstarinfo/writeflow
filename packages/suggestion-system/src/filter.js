...
export function filterSuggestions(suggestions, options = {}) {
    const { minConfidence = 0.60, ignoredRuleIds = new Set(), ignoredWords = new Set(), personalDictionary = new Set(), allowLowConfidence = false } = options;
    return suggestions.filter(s => {
        // Check confidence threshold
        const tier = getConfidenceTier(s.confidence);
        if (tier === 'hidden')
            return false;
        if (tier === 'low' && !allowLowConfidence)
            return false;
        if (s.confidence < minConfidence)
            return false;
        // Check ignored rules
        if (s.ruleId && ignoredRuleIds.has(s.ruleId))
            return false;
        // Check ignored words (case-insensitive)
        const lowerOriginal = s.original.toLowerCase().trim();
        if (ignoredWords.has(lowerOriginal))
            return false;
        // Check personal dictionary for spelling errors
        if (s.type === 'spelling' && personalDictionary.has(lowerOriginal))
            return false;
        return true;
    });
}
