import { wordNetProvider } from '@writeflow/engine-wordnet';
import { detectPosFromContext, disambiguateSynset, scoreCandidate } from './ranker.js';
const POS_NAMES = {
    n: 'Noun',
    v: 'Verb',
    a: 'Adjective',
...
export class SynonymEngine {
    getSynonyms(word, sentenceContext = '', options = {}) {
        ...
        for (const synset of allSynsets) {
            const synsetCat = synset.category || 'similar';
            const synsetLevel = synset.level || 'natural';
            if (synset.antonyms) {
                synset.antonyms.forEach((a) => antonymsSet.add(a));
            }
...
//# sourceMappingURL=synonym-engine.js.map