import { DocumentModel, Paragraph, Sentence, Token, SelectionRange } from './types.js';
...
export function createDocumentModel(
  rawText: string = '',
  id: string = `doc_${Date.now()}_${++docIdCounter}`,
  language: string = 'en',
...
export function updateDocumentModel(
  doc: DocumentModel,
  newText: string,
...
export function getWordAtOffset(doc: DocumentModel, offset: number): Token | null {
...
export function getSentenceAtOffset(doc: DocumentModel, offset: number): Sentence | null {
  for (const sentence of doc.sentences) {
    if (offset >= sentence.start && offset <= sentence.end) {
      return sentence;
    }
